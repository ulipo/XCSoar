...
SystemWidget::SwitchKernel()
{
  ...
  if (model != KoboModel::MINI &&
      ...
      ShowMessageBox("Funzione pensata per Kobo Mini, Touch 2.0, Glo HD e Aura 2, ma questo non e' uno di quelli. Procedi a tuo rischio.  Continuare?",
  ...
  if (!KoboInstallKernel(kernel_image)) {
      ShowMessageBox("Impossibile attivare il kernel.", _("Errore"), MB_OK);
      return;
  }
...
{
  if (!KoboUmountData()) {
      ShowMessageBox("Impossibile smontare la partizione dati.", _("Errore"),
                     MB_OK);
      return;
  }

  if (!KoboExportUSBStorage()) {
      ShowMessageBox("Impossibile esportare la partizione dati.", _("Errore"),
                     MB_OK);
      KoboMountData();
      return;
  }

  ShowMessageBox("Il tuo PC avra' accesso alla memoria del Kobo finche' questa finestra di dialogo rimarra' aperta.",
                 "Esporta memoria USB",
                 MB_OK);
